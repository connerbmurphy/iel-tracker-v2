...
import { tenurePoints, rolePoints, fmtMoney2, hoursFromPunch, monthStr, fmtDate } from '../utils/helpers';
...
const STATUSES = { 'Lead':['Active'], 'Foreman':['Qualified'], 'Install Tech':['Experienced','In Training'] };
const ALL_STATUSES = ['In Training','Experienced','Qualified','Active'];
const EMP_ID_COUNTER = () => `IEL-${String(Date.now()).slice(-4)}`;

export default function EmployeesScreen() {
  const { employees, managerUnlocked } = useApp();
...
function AddEmployeeForm() {
  ...
  return (
    <>
      <Toast message={toast} />
      {!open && <button style={{ ...S.addChip, width:'100%', justifyContent:'center', marginTop:8 }} onClick={() => setOpen(true)}><Plus size={13}/> Add employee</button>}
      {open && (
        <Card>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:14 }}>New Employee</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div style={{ gridColumn:'span 2' }}><div style={S.hourLabel}>Full name</div><input style={{ ...S.textInput, flex:'unset', width:'100%' }} value={form.name} onChange={e=>set('name',e.target.value)} placeholder="First and last name" /></div>
            <div><div style={S.hourLabel}>Employee ID</div><input style={{ ...S.textInput, flex:'unset', width:'100%' }} value={form.employeeId} onChange={e=>set('employeeId',e.target.value)} /></div>
            <div><div style={S.hourLabel}>Hire date</div><input type="date" style={S.timeInput} value={form.hireDate} onChange={e=>set('hireDate',e.target.value)} /></div>
            <div><div style={S.hourLabel}>Role</div>
            ...
            <div style={{ gridColumn:'span 2' }}><div style={S.hourLabel}>Burdened rate ($/hr) - manager only</div><NumberInput value={form.burdenedRate} onChange={v=>set('burdenedRate',v)} suffix="$/hr" /></div>
          </div>
  ...
  return (
    <div style={S.screen}>
      ...
      {editing ? (
        <Card>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div style={{ gridColumn:'span 2' }}><div style={S.hourLabel}>Full name</div><input style={{ ...S.textInput, flex:'unset', width:'100%' }} value={form.name} onChange={e=>set('name',e.target.value)} /></div>
            <div><div style={S.hourLabel}>Employee ID</div><input style={{ ...S.textInput, flex:'unset', width:'100%' }} value={form.employeeId} onChange={e=>set('employeeId',e.target.value)} /></div>
            <div><div style={S.hourLabel}>Hire date</div><input type="date" style={S.timeInput} value={form.hireDate} onChange={e=>set('hireDate',e.target.value)} /></div>
            <div><div style={S.hourLabel}>Role</div><select style={{ ...S.select, flex:'unset', width:'100%' }} value={form.role} onChange={e=>set('role',e.target.value)}>{ROLES.map(r=><option key={r} value={r}>{r}</option>)}</select></div>
            <div><div style={S.hourLabel}>Status</div><select style={{ ...S.select, flex:'unset', width:'100%' }} value={form.status} onChange={e=>set('status',e.target.value)}>{ALL_STATUSES.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
            {managerUnlocked && <div style={{ gridColumn:'span 2' }}><div style={S.hourLabel}>Burdened rate ($/hr)</div><NumberInput value={form.burdenedRate} onChange={v=>set('burdenedRate',v)} suffix="$/hr" /></div>}
          </div>
      ...
      {open && (
        <Card>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div><div style={S.hourLabel}>Status</div>
              <select style={{ ...S.select, flex:'unset', width:'100%' }} value={form.status} onChange={e=>set('status',e.target.value)}>
                {(STATUSES[form.role]||[]).map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div style={{ gridColumn:'span 2' }}><div style={S.hourLabel}>Burdened rate ($/hr) - manager only</div><NumberInput value={form.burdenedRate} onChange={v=>set('burdenedRate',v)} suffix="$/hr" /></div>
          </div>
      ...
      {editing ? (
        <Card>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div><div style={S.hourLabel}>Hire date</div><input type="date" style={S.timeInput} value={form.hireDate} onChange={e=>set('hireDate',e.target.value)} /></div>
            <div><div style={S.hourLabel}>Role</div><select style={{ ...S.select, flex:'unset', width:'100%' }} value={form.role} onChange={e=>set('role',e.target.value)}>{ROLES.map(r=><option key={r} value={r}>{r}</option>)}</select></div>
            <div><div style={S.hourLabel}>Status</div><select style={{ ...S.select, flex:'unset', width:'100%' }} value={form.status} onChange={e=>set('status',e.target.value)}>{(STATUSES[form.role]||[]).map(s=><option key={s} value={s}>{s}</option>)}</select></div>
            {managerUnlocked && <div style={{ gridColumn:'span 2' }}><div style={S.hourLabel}>Burdened rate ($/hr)</div><NumberInput value={form.burdenedRate} onChange={v=>set('burdenedRate',v)} suffix="$/hr" /></div>}
          </div>
          <button style={{ ...S.btnPrimary, width:'100%', marginTop:14, padding:'12px 0', fontSize:14 }} onClick={save}>Save changes</button>
        </Card>
